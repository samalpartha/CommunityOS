...
const LoginScreen: React.FC<LoginScreenProps> = ({ onLogin, onDemoTour }) => {
    ...
    // Initial check for redirect results (legacy support)
    useEffect(() => {
                ...
                <div className="w-full md:w-1/2 p-8 md:p-12 flex flex-col justify-center">
                    ...
                    <div className="grid grid-cols-4 gap-3 mb-8">
                        <button
                            ...
                            disabled={isLoading}
                            className={`flex items-center justify-center p-3 border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors transform active:scale-95 ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}>
                            <GoogleIcon />
                        </button>
